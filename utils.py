...
def compute_map(ranks, gnd, kappas=[]):
    ...
    Arguments
    ---------
    ranks  : list of lists of ranks of retrieved images for each query
    gnd    : list of ground truth dictionaries for each query
    kappas : list of k values for which to compute map, e.g., [1, 5, 10]
    ...
    map = 0.
    for qid in range(len(gnd)):
        qgnd = gnd[qid]
        qranks = ranks[qid]
        
        ap = compute_ap(qranks, len(qgnd['ok']))
        map += ap
...
class DataAugmentationDINO(object):
    """
    Data augmentation for DINO training.
    """
    def __init__(
        self,
        global_crops_scale,
        ...
        flip_and_color_jitter = tf.Compose([
            ...
            tf.RandomApply(
                [tf.ColorJitter(brightness=0.4*color_jitter_scale, 
                               contrast=0.4*color_jitter_scale, 
                               saturation=0.2*color_jitter_scale, 
                               hue=0.1*color_jitter_scale)],
                p=0.8
            ),
        ...
        for i in range(global_crops_number):
            global_crops_aug.append(tf.Compose(augmentation))
        ...
        for i in range(local_crops_number):
            local_crops_aug.append(tf.Compose(local_transfo))
            
        self.global_crops_aug = global_crops_aug
        self.local_crops_aug = local_crops_aug

    def __call__(self, image):
        crops = []
        for op in self.global_crops_aug:
            crops.append(op(image))
        for op in self.local_crops_aug:
            crops.append(op(image))
        return crops
